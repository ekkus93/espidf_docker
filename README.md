...
A dockerize version of idf for esp-idf.
...
#### Set COM port for esp32
The `Set-Esp32Port.ps1` helper handles detection, prompts, and sets `ESPPORT` for you.
...
#### Normal Workflow
After you have everything set up for Windows, this is the normal workflow.
1. Start Windows
2. From the Start Menu, launch Ubuntu
3. Plug in your esp32
4. Open an **Admin** Powershell terminal a run the following command to map your COMM:
```powershell
cd C:/espidf_docker/esp-idf/tools
powershell -NoProfile -ExecutionPolicy Bypass -File .\Set-Esp32Port.ps1 -Distro Ubuntu
```
5. Open a regular Powershell window
6. If you didn't permanently set up your path environment variables, run:
```powershell
export IDF_PATH=/opt/espidf_docker/esp-idf
export PATH=$IDF_PATH/tools:$IDF_PATH/components/esptool_py/esptool:$PATH
```
7. In that Powershell window, you can run the following commands:
```powershell
idf.ps1 fullclean
idf.ps1 build
idf.ps1 flash
idf.ps1 monitor
```
The other available commands for idf should also work.